...
2. **渲染引擎**（三级降级策略）：
   - **主要方案**：SVG生成器（轻量、快速、矢量输出）
   - **备选方案**：Remotion（基于React的高质量渲染）
   - **降级方案**：HTML+Puppeteer（当其他方案不可用时）
3. **模板系统**：
   - SVG模板：8种专业风格，支持矢量编辑
   - React组件模板：复杂动画效果
   - HTML模板：兼容性保证
...
- ⚡ **极速渲染**：纯代码生成，无需浏览器
- 🎨 **矢量输出**：SVG可编辑，无限缩放
- 📐 **专业布局**：参考fireworks-tech-graph的布局算法
- 🎯 **语义化样式**：8种完整设计风格
- 💾 **轻量依赖**：仅需rsvg-convert导出PNG

**文件结构**：
```
infographic/{topic-slug}/
├── {topic-slug}.png          # 最终PNG图片
├── {topic-slug}.svg          # SVG矢量文件（可选）
