...
async function renderFromConfig(config, outputPath) {
  ...
  // 方案1: 优先使用新的SVG生成器（轻量、快速、高质量）
  try {
    console.log(`  - 尝试使用SVG生成器渲染...`);
    const { generateInfographic } = require('./svg-generator');

    // 转换配置格式
    const svgConfig = {
      type: config.template || 'knowledge',
      style: mapStyleToId(config.style),
      dimensions: {
        width: config.output_config?.width || config.output?.width || 1920,
        height: config.output_config?.height || config.output?.height || 1080
      },
      content: config.content || config
    };

    const result = generateInfographic(svgConfig.content, svgConfig);

    // 导出PNG
    const tempSvgPath = outputPathFull.replace('.png', '.svg');
    result.save(tempSvgPath);

    // 使用rsvg-convert导出PNG
    const { execSync } = require('child_process');
    execSync(`rsvg-convert -w ${svgConfig.dimensions.width} "${tempSvgPath}" -o "${outputPathFull}"`, { stdio: 'pipe' });
    ...
    console.log('  ✅ SVG生成器渲染成功！');
    return { outputPath: outputPathFull, renderer: 'svg-generator' };
  } catch (svgError) {
    console.log(`  ⚠️  SVG生成器渲染失败: ${svgError.message}`);
    console.log(`  - 降级到备选方案...`);
  }
  ...
  try {
    ...
    try {
      ...
      console.error(`  - SVG错误: ${svgError?.message || 'N/A'}`);
      console.error(`  - Remotion错误: ${error.message}`);
      console.error(`  - HTML错误: ${htmlError.message}`);
      throw new Error(`所有渲染方式都失败`);
    }
  }
}
 ...
 * 映射风格名称到ID
 */
function mapStyleToId(style) {
  if (typeof style === 'number') return style;
  if (typeof style !== 'string') return 1;

  const styleMap = {
    'minimal': 1, '极简': 1, '极简风': 1,
    'macaron': 2, 'cute': 2, '马卡龙': 2, '马卡龙风': 2, '可爱': 2, '可爱风': 2,
    'cyberpunk': 3, '赛博朋克': 3, '科技': 3, '科技风': 3,
    'morandi': 4, '莫兰迪': 4, '莫兰迪风': 4, '优雅': 4,
    'chinese': 5, '国潮': 5, '国潮风': 5, '传统': 5,
    'business': 6, '商务': 6, '商务风': 6, '专业': 6,
    'handdrawn': 7, '手绘': 7, '手绘风': 7,
    'flat': 8, '扁平化': 8, '扁平': 8
  };
